...
DEFAULT_INPUT = project_data_dir("embeddings", "embeddings_labelled.parquet")
...
def build_target(
    ...
    if raw.isna().any():
        raise ValueError(
            f"{int(raw.isna().sum())} plots have no interpreted transition. "
            "Drop them before building the target -- pooling them into "
            f"'{RARE_LABEL}' would model unlabelled plots as a real class."
        )
...
def main() -> None:
    ...
    # Plots the sampler never interpreted carry no target. They are dropped
    # rather than pooled into the rare class, which would train the model to
    # call unlabelled plots a transition.
