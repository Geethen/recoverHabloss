...
lk = pd.read_csv(project_data_dir('scratch') / 'lookup.csv')
...
for f,b in [('samples_biomes_cleaned','biomes_main'),('samples_biomes_extra_cleaned','biomes_extra'),
    ...
    g=gpd.read_file(R / 'data' / 'from_gee' / f'{f}.geojson')[['PLOTID','stratum']]; g['batch']=b; cl.append(g)
...
fs=glob.glob(str(R / 'data' / 'from_gee' / 'areas_biomes_with_buffer' / '*.csv'))
dfs=[]
for f in fs:
    try:
        d=pd.read_csv(f)
